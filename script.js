...
var PROJECTS = {
  res: [
    ...
    { name:'Courtyard by Narang Reality', thumb:B+'3e7de2_11482d322cb04ac4aa52ab42cc489bb6~mv2.png', concept:[B+'3e7de2_476adfcc728d491bb7551dc76696283c~mv2.jpeg'], build:[B+'3e7de2_c8df70f8a556413d93c0d38d5a8f9020~mv2.jpeg'] },
...
function renderProjects(sk) {
  ...
  for (var i = 0; i < list.length; i++) {
    (function(p, idx) {
      ...
      tag.className = 'imm-sector-tag ' + m.tag;
...
(function() {
  ...
  var SHAPES = ['circle','circle','circle','square','cross'];

  function resize() { W = canvas.width = window.innerWidth; H = canvas.height = window.innerHeight; }
  window.addEventListener('resize', resize);
  resize();

  for (var i = 0; i < (IS_TOUCH ? 25 : 70); i++) {
    particles.push({
      ...
      wobbleSpeed: 0.003 + Math.random() * 0.009,
  ...
  function drawParticle(p) {
    ...
    else if (p.shape === 'square') { var s = p.r * 1.6; ctx.fillRect(-s/2, -s/2, s, s); }
    else if (p.shape === 'cross') {
      var t = p.r * 0.35, l = p.r * 1.4; ctx.lineWidth = t; ctx.lineCap = 'round';
  ...
  function draw() {
    ...
    particles.forEach(function(p) {
      ...
      p.x += p.vx + Math.sin(p.wobble) * 0.25; p.y += p.vy;
...
function openLB(sk, idx) {
  ...
  if (p.build && p.build.length) {
    ...
    bb.onclick = function() { showImg(lbConceptCount); };
    tr.appendChild(bb);
  }
  showImg(0);
...
(function() {
  function splitAndAnimate(el, delay) {
    ...
    el.innerHTML = words.map(function(w) { return '<span class="word-wrap"><span class="word-inner">' + w + '</span></span>'; }).join(' ');
    el.querySelectorAll('.word-inner').forEach(function(w, i) { w.style.transitionDelay = (delay + i * 0.06) + 's'; });
  }
  ...
  document.querySelectorAll('.step-card').forEach(function(el, i) {
    el.style.cssText += 'opacity:0;transform:translateY(40px);transition:opacity .7s ease ' + (i*0.12) + 's,transform .7s cubic-bezier(.16,1,.3,1) ' + (i*0.12) + 's;';
    var o3 = new IntersectionObserver(function(entries) {
      entries.forEach(function(e) { if (e.isIntersecting) { el.style.opacity='1'; el.style.transform='translateY(0)'; o3.unobserve(el); } });
    }, { threshold: 0.15 });
    o3.observe(el);
  });
})();
